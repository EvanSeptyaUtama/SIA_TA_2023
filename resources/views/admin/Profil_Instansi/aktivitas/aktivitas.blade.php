                            ...
                            <tbody>
                                @foreach ($data_aktivitas as $data)
                                    <tr>
                                        ...
                                        <td>
                                            <img src="{{ url('storage/Kegiatan_Sekolah/' . $data->gambar_aktivitas) }}" class="rounded" style="width: 100px">
                                        </td>
                                        <td class="text-center">
                                            ...
                                            <form action="{{route('edit_aktivitas',$data)}}" method="get">
                                                <button type="submit" class="btn btn-sm btn-info mt-2">
                                                    Edit 
                                                </button>
                                            </form>
                                            <form action="{{ route('hapus_aktivitas', $data->id) }}" method="post">
                                                @csrf
                                                @method('delete')
