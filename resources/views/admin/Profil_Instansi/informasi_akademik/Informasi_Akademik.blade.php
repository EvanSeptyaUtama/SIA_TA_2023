                    ...
                    <tbody>
                        @foreach ($data_informasi_akademik as $data)
                            <tr>
                                ...
                                <td>
                                    <img src="{{ url('storage/Informasi_Akademik/' . $data->gambar_informasi_akademik) }}" class="rounded" style="width: 100px">
                                </td>
                                <td class="text-center">
                                    <div>
                                        <form action="{{route('tampil_informasi_akademik', $data)}}" method="get">
                                            <button type="submit" class="btn btn-sm btn-secondary mt-2">
                                                Lihat
                                            </button>
                                        </form>
                                        <form action="{{route('edit_informasi_akademik',$data)}}" method="get">
                                            <button type="submit" class="btn btn-sm btn-info mt-2">
                                                Edit
                                            </button>
                                        </form>
                                    <form action="{{ route('hapus_informasi_akademik', $data->id) }}" method="post">
                                            @csrf
                                            @method('delete')
