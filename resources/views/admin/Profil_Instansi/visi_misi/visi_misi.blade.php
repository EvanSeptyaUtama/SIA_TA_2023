                ...
                <tbody>
                    @foreach ($data_visi_misi as $data)
                        <tr>
                            ...
                            <td>{{$data->misi}}</td>
                            <td>
                                <img src="{{ url('storage/Visi_Misi/' . $data->gambar_visi_misi) }}" class="rounded" style="width: 100px">
                            </td>
                            <td class="text-center">
                                <div>
                                    ...
                                    <form action="{{route('edit_visi_misi',$data)}}" method="get">
                                        <button type="submit" class="btn btn-sm btn-info mt-2">
                                            Edit
                                        </button>
                                    </form>
                                   <form action="{{ route('hapus_visi_misi', $data->id) }}" method="post">
                                        @csrf
                                        @method('delete')
