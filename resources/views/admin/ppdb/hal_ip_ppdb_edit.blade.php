...
<div class="m-2">
    <div class="container">
        ...
        <div class="card">
            <div class="card-header text-center">{{ __('Data Informasi Pengumuman') }}</div>

            <div class="card-body">
                <form action="{{route('update_halaman_ip_ppdb', $data_ip_ppdb)}}" method="post" enctype="multipart/form-data">
                    @method('patch')
                    @csrf 
                    <div class="row mb-4">
                        <label>Judul</label>
                        <input type="text" name="judul_informasi_pengumuman" value="{{$data_ip_ppdb->judul_informasi_pengumuman}}" class="form-control">
                    </div>
